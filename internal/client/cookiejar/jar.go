...
// Entry is the complete serializable state of one cookie. It intentionally
// mirrors the jar's internal entry so persistence does not depend on the
// outgoing Cookies view, which omits scope and lifetime metadata.
type Entry struct {
	Name       string
	Value      string
	Quoted     bool
...
// Snapshot returns a detached copy of all cookie state.
func (j *Jar) Snapshot() map[string]map[string]Entry {
	j.mu.Lock()
	defer j.mu.Unlock()

	result := make(map[string]map[string]Entry, len(j.entries))
	for key, entries := range j.entries {
		result[key] = make(map[string]Entry, len(entries))
		for id, e := range entries {
			result[key][id] = Entry{Name: e.Name, Value: e.Value, Quoted: e.Quoted, Domain: e.Domain, Path: e.Path, SameSite: e.SameSite, Secure: e.Secure, HttpOnly: e.HttpOnly, Persistent: e.Persistent, HostOnly: e.HostOnly, Expires: e.Expires, Creation: e.Creation, LastAccess: e.LastAccess, SeqNum: e.seqNum}
		}
	}
	return result
}

// Restore replaces the jar with a validated persisted snapshot.
func (j *Jar) Restore(snapshot map[string]map[string]Entry) error {
	j.mu.Lock()
	defer j.mu.Unlock()

	entries := make(map[string]map[string]entry, len(snapshot))
	var next uint64
	for key, stored := range snapshot {
		entries[key] = make(map[string]entry, len(stored))
		for id, e := range stored {
			if e.Name == "" || e.Domain == "" || e.Path == "" || id != fmt.Sprintf("%s;%s;%s", e.Domain, e.Path, e.Name) || key != jarKey(e.Domain, j.psList) {
				return errors.New("invalid persisted cookie identity")
			}
			if e.Creation.IsZero() || e.LastAccess.IsZero() || (e.Persistent && e.Expires.IsZero()) {
				return errors.New("invalid persisted cookie metadata")
			}
			entries[key][id] = entry{Name: e.Name, Value: e.Value, Quoted: e.Quoted, Domain: e.Domain, Path: e.Path, SameSite: e.SameSite, Secure: e.Secure, HttpOnly: e.HttpOnly, Persistent: e.Persistent, HostOnly: e.HostOnly, Expires: e.Expires, Creation: e.Creation, LastAccess: e.LastAccess, seqNum: e.SeqNum}
			if e.SeqNum >= next {
				next = e.SeqNum + 1
			}
		}
	}
	j.entries, j.nextSeqNum = entries, next
	return nil
}
